...
```dart
   ...
   final imagesPath = await CunningDocumentScanner.getPictures(
      ...
      androidOptions: AndroidScannerOptions(
      ...
      iosScannerOptions: IosScannerOptions(
         ...
         isAutoScanAllowed: false,
         isFlashAllowed: false
         backgroundColor: Color(0xFF333333).toARGB32()
         tintColor: Colors.white.toARGB32()
      ),
   );
```

Notice: Because WeScan still not support `batch scanning`, so if you specific `noOfPages` other than `1`, WeScan is never active for use.
